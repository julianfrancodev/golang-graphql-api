...
func initializeAPI() (*chi.Mux, *postgres.Db) {
	...
	// Create a new connection to our pg postgres
	db, err := postgres.New(
		postgres.ConnString("127.0.0.1", 5432, "postgres", "go-graphql"),
	)
	if err != nil {
		log.Fatal(err)
	}
	...
	// Create a server struct that holds a pointer to our postgres as well
